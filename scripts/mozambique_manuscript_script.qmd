...
-   to assess the difference in number of contacts between day 1 and 2, we use the Wilcoxon rank-sum test of significance for paired data

### Determinants of contact outliers (PP)
...
label(participants$contacts_completedbylb) <- "Diary filled by"
...
First, we report the total and mean (and median, see table) number of contacts collected over two days. The mean number of contacts reported was `r mean_contacts_overall$mean` (95% CI `r ci_contacts_overall$ll` - `r ci_contacts_overall$ul`). Overall, we observe a significant difference in the mean number of contacts reported on day 1 compared to day 2 (Wilcoxon rank-sum test p=`r round(overall_d1d2_wilcoxtest$p.value, 2)`).

@fig-overall-contact-distribution shows the distribution of *all* reported contacts stratified by study site. The mean (95% CI) contacts for the rural and urban sites was `r mean_contacts_site$mean[1]` (95% CI `r ci_contacts_rural$ll` - `r ci_contacts_rural$ul`) and `r mean_contacts_site$mean[2]` (95% CI `r ci_contacts_urban$ll`-`r ci_contacts_urban$ul`), respectively. The rural area reported higher number of mean contacts compared to the urban area.
...
::: callout-important
From here below, I only present summaries of contacts for day 1 only.
:::
...
@fig-contact-distribution-d1 shows the distribution of the contacts by rural and urban site and the corresponding mean contact rates on day 1 only. The rural mean contact rate was higher than the urban rate (`r mean_contacts_site_d1[1,2]` vs `r mean_contacts_site_d1[2,2]`, `r round(bysite_d1_wilcoxtest$p.value, 2)`).
...
fig_contacts_age_box <- ggplot(contacts_age, aes(x = participant_age, y = num_contacts, fill = study_site)) +
...
contacts_touch <- contacts_type %>% filter(touch_contact == "Yes")
contacts_physical <- contacts_type %>% filter(touch_contact == "No")
mean_contacts_touch <- mean(contacts_touch$num_contacts)
mean_contacts_physical <- mean(contacts_physical$num_contacts)
wilcoxtest_bytouch <- wilcox.test(contacts_touch$num_contacts, contacts_physical$num_contacts) # p<0.05
