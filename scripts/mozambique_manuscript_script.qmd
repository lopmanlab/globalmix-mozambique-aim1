...
```{r additional-cleaning-participants}
#| include: false
#| warning: false
#| echo: false

mean_hh_size <- round(mean(participants$hh_size %>% na.omit()),1) # omits 1
...
df_contact_d1 <- left_join(df_contact_d1, day1_num_contacts %>% select(-study_site), by = "rec_id" )

# mean overall number of contacts on day 1 only
ci_contacts_overall_d1 <- lm(num_contacts ~ 1, day1_num_contacts)
mean_contacts_overall_d1 <- ci_contacts_overall_d1$coefficients
#   df_contact_d1 %>%
#   dplyr::summarize(mean = round(mean(num_contacts),1), .groups = "drop")
...
# we see that not.
# so, we check which records are in d1 not in d2 and vice versa, append these ang
# assign a value of 0.
...
overall_d1d2_wilcoxtest <- wilcox.test(day1_num_contacts$num_contacts, 
...
bysite_d1_wilcoxtest <- wilcox.test(rural_day1_contacts $num_contacts, 
                                    urban_day1_contacts $num_contacts)
...
wilcoxtest_bysex <- wilcox.test(contacts_male$num_contacts, contacts_female$num_contacts) # p=0.03


fig_sex_box <- fxn_fig_boxplot(contacts_sex %>% filter(!is.na(participant_sex)), "participant_sex")
...
mean_contacts_conv_rural <- mean(contacts_physical_rural$num_contacts)
mean_contacts_touch_urban <- mean(contacts_touch_urban$num_contacts)
mean_contacts_conv_urban <- mean(contacts_physical_urban$num_contacts)

wilcoxtest_bytouch_rural <- wilcox.test(contacts_touch_rural$num_contacts, 
                                        contacts_conv_rural$num_contacts) # p<0.05

wilcoxtest_bytouch_urban <- wilcox.test(contacts_touch_urban$num_contacts, 
                                        contacts_conv_urban$num_contacts) # p<0.05
...
wilcoxtest_byhhmembership <- wilcox.test(hhmember$num_contacts, nonmember$num_contacts) # p<0.05
...
inschool <- contacts_education %>% filter(enrolled_school == "Yes")
notinschool <- contacts_education %>% filter(enrolled_school == "No")
mean_contacts_inschool <- mean(inschool$num_contacts)
mean_contacts_notinschool <- mean(notinschool$num_contacts)
wilcoxtest_byschool <- wilcox.test(inschool$num_contacts, notinschool$num_contacts) # p<0.05
...
# Ho: there is no difference in number of contacts between those in school vs not in school       
weekday <- contacts_weekday %>% filter(weekday == "Weekday")
weekend <- contacts_weekday %>% filter(weekday == "Weekend")
mean_contacts_weekday <- mean(weekday$num_contacts)
mean_contacts_weekend <- mean(weekend$num_contacts)
wilcoxtest_byweekday <- wilcox.test(weekday$num_contacts, weekend$num_contacts) # p<0.05
...
# fxn_weekday_boxplot <- function(data, site) {
#   # Filter the data based on the provided study_site
#   filtered_data <- data %>%
#     dplyr::filter(study_site == site)
#   
#   # Create the ggplot boxplot
#   plot <- ggplot(filtered_data, aes(x = participant_age, y = num_contacts, fill = weekday)) +
#     geom_boxplot(position = position_dodge(width = 0.8)) +
#     scale_fill_manual(values = c("#ef8a62", "#67a9cf")) +
#     labs(x = "Participant age", y = "Number of contacts") +
#     ylim(0, 40) +
...
# }
# 
# fig7_weekday_rural_box <- fxn_weekday_boxplot(contacts_weekday, site = "Rural")
# fig7_weekday_urban_box <- fxn_weekday_boxplot(contacts_weekday, site = "Urban")
# 
# fig7_weekday_box <- fig7_weekday_rural_box/fig7_weekday_urban_box +  plot_layout(ncol = 1, nrow = 2)
# fig7_weekday_box
...
ari <- contacts_ari %>% filter(ari_symptom == ">1 symptom")
noari <- contacts_ari %>% filter(ari_symptom == "No symptom")
mean_contacts_ari <- mean(ari$num_contacts)
mean_contacts_noari <- mean(noari$num_contacts)
wilcoxtest_byari <- wilcox.test(ari$num_contacts, noari$num_contacts) # p=0.50
...
The mean household size was `r mean_hh_size`. Occupation categories were mainly invariable by site, with majority being  students at various levels (30%), children aged <5 years (17%), or unemployed adults (15%). Key differences were observed among business people (rural 1%, urban 11%) and office workers (rural 5%, urban 10%). When asked about regular mask use inside or outside the house, 931 (76%) of participants reported wearing a mask with no difference by site. Finally in both sites, almost two-thirds of the participants (701, 57%) were able to read and write and half of all participants required assistance from a fieldworker to fill in the diary at the end of the two days.  

::: callout-note
Talk about: - household size, distribution of number of rooms - diary keeping by participant/ shadow - any significant comments regarding challenges experienced during diary keeping? - any changes in behavior reported? - vaccination status?
:::
```{r}
table1
```
...
contact_hist_overall
```
First, we report the total and mean (and median, see SI 2) number of contacts collected over two days. Overall, the mean number of contacts reported on two days was `r mean_contacts_overall$mean` (95% CI `r ci_contacts_overall$ll`-`r ci_contacts_overall$ul`). We observe no difference in the mean number of contacts reported on day 1 (`r mean_contacts_overall_d1` (95% CI `r ci_contacts_overall_d1$ll`-`r ci_contacts_overall_d1$ul`) compared to day 2 (`r mean_contacts_overall_d2` (95% CI `r ci_contacts_overall_d2$ll`-`r ci_contacts_overall_d2$ul`) (Wilcoxon rank-sum test p=`r round(overall_d1d2_wilcoxtest$p.value, 2)`), thus from here onwards we only report the number of contacts occuring on day 1.

@fig-baseline-distributions shows the distribution of the contacts by rural (C) and urban (D) site and the corresponding mean contact rates on day 1 only. The rural mean contact rate (`r mean_contacts_site_d1[1,2]` (`r ci_contacts_rural_d1$ll`-`r ci_contacts_rural_d1$ul`)) was higher than the urban rate (`r mean_contacts_site_d1[2,2]`  (95% CI `r ci_contacts_urban_d1$ll`-`r ci_contacts_urban_d1$ul`) (p<0.05).


::: callout-note
Additional analysis can include: \~1. statistical tests\~ \~2. mean number of contacts over both days\~ 3. mean number of unique contacts contacts only over both days 4. mean number of unique contacts reported on both day 1 and 2
:::


### Distribution of contacts by covariates:
...
## 1. Number of contacts reported over two days

```{r}
table2
```
## 2. Median (IQR) rural contacts over two days

```{r}
table3
```
## 3. Median (IQR) urban contacts over two days
