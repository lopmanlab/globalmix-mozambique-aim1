...
# import raw data
data <- readRDS("../raw/all_data_09142022.RDS")
...
label(exit$contacts_completedbylb) <- "Who filled the diary?" 
...
exit$all_contactslb <- factor(exit$all_contactslb, levels = c("Yes", "No", 
                                                              "No response"))
label(exit$all_contactslb) <- "Were you able to include every single contact?"

exit <- exit %>% 
  dplyr::mutate(contact_comparelb = case_when(contact_compare == 1 ~ "Fewer", # Much fewer contacts then usual
...
exit$contact_comparelb <- factor(exit$contact_comparelb, levels = c("Fewer", "Same", "More"))
label(exit$contact_comparelb) <- "How did the 2 days on which you reported your contact compare to a typical day?"
...
exit$behavior_changelb <- factor(exit$behavior_changelb, levels = c("Increased interactions", "No change",  
                                                                    "Reduced interactions", "No response"))
label(exit$behavior_changelb) <- "How has your social behavior has changed following the start of COVID-19?" 
...
exit$movement_changelb <- factor(exit$movement_changelb, levels = c("Increased", "No change",  
                                                                    "Reduced", "No response"))
...
exit$days_left_homelb <- factor(exit$days_left_homelb, levels = c("0", "1-3", "4-7", 
                                                                  ">7", "No response"))
...
exit$yesterday_left_homelb <- factor(exit$yesterday_left_homelb, 
                                     levels = c("Yes", "No"))
...
exit$quarantinelb <- factor(exit$quarantinelb, levels = c("Yes", "No",
                                                          "No response"))
...
exit$self_isolationlb <- factor(exit$self_isolationlb, levels = c("Yes", "No",
                                                                  "No response"))
...
exit$isolation_abilitylb <- factor(exit$isolation_abilitylb, levels = c("Yes", "No"))
...
exit$mask_uselb <- factor(exit$mask_uselb, levels = c("Yes", "No", "No response"))
...
exit$contact_with_infectedlb <- factor(exit$contact_with_infectedlb, levels = c("Yes", "No",
                                                                                "No response"))
...
exit$self_covid_testlb <- factor(exit$self_covid_testlb, 
...
exit$family_covid_symptomslb <- factor(exit$family_covid_symptomslb, levels = c("Yes", "No",
                                                                                "No response"))
...
exit$family_covid_testlb <- factor(exit$family_covid_testlb, levels = c("Yes", "No",
                                                                        "No response"))
...
exit <- exit %>% 
  dplyr::mutate(phone_accesslb = case_when(phone_access == 0 ~ "No",
                                           phone_access == 1 ~ "Yes"))
exit$phone_accesslb <- factor(exit$phone_accesslb, levels = c("Yes", "No"))
...
exit$phone_ownershiplb <- factor(exit$phone_ownershiplb, levels = c("Yes", "No"))
...
#(internet enabled)"))
label(exit$phone_functionlb) <- "If you use a mobile phone, what is the functionality? "

exit <- exit %>% 
  dplyr::mutate(phone_comm_modelb = case_when(phone_comm_mode == 1 ~ "Normal call and text",
                                              phone_comm_mode == 2 ~ "Internet calls and text"))
#e.g. via Whatsapp"))
exit$phone_comm_modelb <- factor(exit$phone_comm_mode, levels = c("Normal call and text",
...
exit <- exit %>% 
  dplyr::mutate(phone_signallb = case_when(phone_signal == 0 ~ "Poor",
                                           ...
                                           phone_signal == 2 ~ "Good"))
exit$phone_signallb <- factor(exit$phone_signallb, levels = c("Poor", "Normal", "Good"))
...
exit$phone_uselb <- factor(exit$phone_uselb, levels = c("Never", "1-5 times", 
                                                        "6-10 times", ">10 times"))
label(exit$phone_uselb) <- "In the last 24 hours, how often did you call/ text using your mobile phone?"
...
exit_vars <- c("rec_id", "contacts_completedbylb", "all_contactslb",
               "contact_comparelb", "behavior_changelb", "movement_changelb", "days_left_homelb",
               "activities___1", "activities___2", "activities___3", "activities___4",
               "activities___5", "activities___6", "activities___7", "activities___8",
               "activities_work", "activities_school", "activities_other", 
               "yesterday_left_homelb", "quarantinelb", "quarantine_behaviorlb", "self_isolationlb",
               "self_isolation_locationlb", "self_isolation_homelb", "self_isolation_awaylb",
               "isolation_abilitylb", "mask_uselb", "age_symptom", "ari_symptom", 
               "quarantine_behaviorlb", "self_isolationlb", "self_isolation_locationlb", 
               "self_isolation_homelb", "self_isolation_awaylb", "isolation_abilitylb",
               "mask_uselb", "ari_symptom", "contact_with_infectedlb", "self_covid_testlb",
               "family_covid_symptomslb", "family_covid_testlb", "phone_accesslb",
               "phone_ownershiplb", "phone_functionlb", "phone_comm_modelb", "phone_signallb",
               "phone_uselb")
...
aim1_summary <- participant %>%
  ...
  select("study_site", "participant_sex", "participant_age", "read_write", 
         ...
         "ari_symptom", "mask_uselb", "age_symptom") %>%
...
contact_hh_all <- contact_hh_all %>%
  ...
  filter(had_contact=="Yes") %>% # household contacts with 10700/15156 (71%) registered members
...
# out of 38260 non-hh contacts reported on d1, 6141 (16%) were repeat.
...
table(contact_hh_all$contact_age, useNA = "always")
#  drop "Missing" and NA
contact_hh_all <- contact_hh_all %>% #15840
  subset(contact_age != "Missing")  # n=345 No response ages

contact_hh_all$age_group_contact = factor(contact_hh_all$contact_age, 
                                          levels = c("<6mo", "6-11mo", "1-4y", "5-9y", 
                                                     "10-14y","15-19y", "20-29y", "30-39y", 
                                                     "40-59y", "60+y")) #, "Missing")) <- dropped and NA
label(contact_hh_all$contact_age) <- "Age group"
...
contact_hh_all = contact_hh_all %>% 
  dplyr::mutate(study_site = case_when(study_site == 1 ~ "Manhiça", 
                                       study_site == 2 ~ "Polana Caniço"))
contact_hh_all$study_site <- factor(contact_hh_all$study_site, levels = c("Manhiça", "Polana Caniço"),
                                    labels = c("Rural", "Urban"))
...
# 1537/10159 (15%) contacts dropped, 88 higher than the final total number of participants
...
contact_nonhh_all = contact_nonhh_all %>% 
  dplyr::mutate(study_site = case_when(study_site == 1 ~ "Manhiça",
                                       study_site == 2 ~ "Polana Caniço"))
contact_nonhh_all$study_site <- factor(contact_nonhh_all$study_site, 
...
contact_nonhh_all$hh_member_relationship <- "Not related"
```
...
contact_vars <- c("rec_id", "study_site", "survey_date", "survey_date_2", 
                  "study_day", "age_group_contact", "contact_sex","touch_contact", 
                  "fromdayone", "repeat_contact_d2", "share_room", "share_bed", 
                  "cook_food", "where_contact","contact_mask", "duration_contact", 
                  "frequency_contact", "known_contact", "hh_member_relationship", 
                  "hh_membership", "relationship", "location_contact___0",
                  "location_contact___1","location_contact___2", "location_contact___3", 
                  "location_contact___4", "location_contact___5","location_contact___6",
                  "location_contact___7","location_contact___8","location_contact___9", 
                  "location_contact___10", "location_contact___11")

participant_contact_hh <- contact_hh_all %>%
  left_join(participant, by = "rec_id")

participant_contact_nonhh <- contact_nonhh_all %>%
  left_join(participant, by = "rec_id")
...
contact_all$day_of_week <- factor(contact_all$day_of_week,
...
contact_all$age_group_contact <- factor(contact_all$age_group_contact, 
                                        levels = c("<6mo", "6-11mo", "1-4y", "5-9y", 
                                                   "10-14y", "15-19y", "20-29y", "30-39y", 
                                                   "40-59y", "60+y"))
...
# participant_vars <- c("rec_id", "aim", "study_site", "date_enrolled", "participant_sex", 
#                       "age", "participant_age", "read_write", "occupation", 
#                       "enrolled_school", "highest_educ", "school_level", 
#                       "transport_use", "hh_resp_exposure", "child_breastfeed", 
#                       "num_sibling")

# merge participants and households file


participants_aim1 <- participant %>% 
  filter(aim == 1)  %>%
  # merge with hh size data
  left_join(hh_size, by = "rec_id")

participants_aim2_index <- participant %>% 
  filter(aim==2, participant_age %in% c("<6mo","6-11mo", "1-4y"))

participants_aim2_index <- participants_aim2_index %>%
  # merge with hh size data
  left_join(hh_size, by = "rec_id")
...
contacts_aim1 <- contacts_aim1 %>%
  dplyr::filter(rec_id %in% unlist(participants_aim1$rec_id))
...
```{r contact-summary-all, echo=FALSE}

# library(gtsummary)
contact_summary_all <- contact_all %>%
  # filter(aim==1) %>%
  select("study_site","contact_sex","age_group_contact","fromdayone", 
         "hh_member_relationship", "hh_membership",
  ...
  bold_labels() %>%
  modify_header(label = "**Variable**") %>%
  modify_caption("**Baseline contact data**")
...
hh_survey <- hh_survey %>% 
  ...
  dplyr::mutate(wash_hands_defecatelb = case_when(wash_hands_defecate == 0 ~ "Never",
                ...
                factor(wash_hands_defecatelb, levels = c("Never", "Rarely", "Sometimes",
                ...
                factor(wash_hands_foodlb, levels = c("Never", "Rarely", "Sometimes", "Always")),
                ...
                factor(wash_hands_toiletlb, levels = c("Never", "Rarely", "Sometimes", "Always")))
...
hh_survey = hh_survey %>% 
  ...
  dplyr::mutate(hh_smokinglb = case_when(hh_smoking == 0 ~ "No",
                ...
                factor(hh_smokinglb, levels = c("Yes", "No", "No response")),
                
                #Do you cook food inside or outside the house?
                hh_cooklb = case_when(hh_cook == 0 ~ "Inside",
                                      hh_cook == 1 ~ "Outside",
                                      hh_cook == 2 ~ "Both"),
                factor(hh_cooklb, levels = c("Inside", "Outside", "Both")))
...
loc_visit_d1 = loc_visit_d1 %>%
  dplyr::mutate(time_visited_d1 = case_when(time_visited_d1 == 0 ~ "<5 mins",
                ...
                factor(time_visited_d1, levels = c("<5 mins", "5-15 mins", "16-30 mins", 
...
loc_visit_d1 = loc_visit_d1 %>%
  dplyr::mutate(visited_frequency_d1 = case_when(visited_frequency_d1 == 0 ~ "Never met before",
                                                 ...
                                                 visited_frequency_d1 == 2 ~ "Daily/ almost daily",
                ...
                factor(visited_frequency_d1, c("Never met before", "Rarely", "Daily/ almost daily",
...
loc_visit_d1$study_day <- 1
```


```{r locations-visited-d2}
loc_visit_d2 <- data %>%
  # filter(rec_id %in% participant_ids) %>%
...
loc_visit_d2 <- loc_visit_d2 %>%
  dplyr::mutate(time_visited_d2 = case_when(time_visited_d2 == 0 ~ "<5 mins",
                ...
                factor(time_visited_d2, levels = c("<5 mins", "5-15 mins", "16-30 mins", 
                                                   "31 mins-1 hr", "1-4 hrs", ">4 hrs")))
...
loc_visit_d2 <- loc_visit_d2 %>%
  dplyr::mutate(visited_frequency_d2 = case_when(visited_frequency_d2 == 0 ~ "Never met before",
                ...
                factor(visited_frequency_d2, c("Never met before", "Rarely", "Daily/ almost daily",
                                               "1-3 times per week", "Once every 2 weeks",
                                               "Once per month", "Once every 3 months")))
...
```{r data-export-rds, echo=FALSE}

# export data
participants_aim1 <- participants_aim1 %>%
  dplyr::left_join(hh_survey_aim1 %>% 
                     select(-c(aim, study_site)), 
                   by = "rec_id")
...
saveRDS(participants_aim2, "/Users/mck/Library/CloudStorage/OneDrive-EmoryUniversity/4.Data and Analysis/GlobalMix/Mozambique/Quantitative/Aim_2/moz_analysis/joint_analysis/data/diary/participant_data_aim2.RDS")

saveRDS(contacts_aim2, "/Users/mck/Library/CloudStorage/OneDrive-EmoryUniversity/4.Data and Analysis/GlobalMix/Mozambique/Quantitative/Aim_2/moz_analysis/joint_analysis/data/diary/contact_data_aim2.RDS")

saveRDS(hh_survey_aim2, "/Users/mck/Library/CloudStorage/OneDrive-EmoryUniversity/4.Data and Analysis/GlobalMix/Mozambique/Quantitative/Aim_2/moz_analysis/joint_analysis/data/diary/household_survey_data_aim2.RDS")

saveRDS(loc_visit_aim2, "/Users/mck/Library/CloudStorage/OneDrive-EmoryUniversity/4.Data and Analysis/GlobalMix/Mozambique/Quantitative/Aim_2/moz_analysis/joint_analysis/data/diary/locations_visited_aim2.RDS")

saveRDS(exit_interview_aim2, "/Users/mck/Library/CloudStorage/OneDrive-EmoryUniversity/4.Data and Analysis/GlobalMix/Mozambique/Quantitative/Aim_2/moz_analysis/joint_analysis/data/diary/exit_interview_aim2.RDS")


# saveRDS(hh_survey_aim2, "/Users/mck/Library/CloudStorage/OneDrive-EmoryUniversity/4.Data and Analysis/GlobalMix/Mozambique/Quantitative/Aim_2/moz_analysis/joint_analysis/data/diary/household_survey_aim2.RDS")


# Export data to GlobalMix shared folder.

EXPORT_PATH <- "~/Library/CloudStorage/OneDrive-EmoryUniversity/4.Data and Analysis/GlobalMix/globalmix-clean-country-data/Mozambique/aim_1/data"

saveRDS(participants_aim1, paste0(EXPORT_PATH, "/participant_data_aim1.RDS"))
saveRDS(participants_aim2, paste0(EXPORT_PATH, "/participant_data_aim2.RDS"))

saveRDS(contacts_aim1, paste0(EXPORT_PATH, "/contact_data_aim1.RDS"))
saveRDS(contacts_aim2, paste0(EXPORT_PATH, "/contact_data_aim2.RDS"))

saveRDS(hh_survey_aim1, paste0(EXPORT_PATH, "/household_survey_aim1.RDS"))
saveRDS(hh_survey_aim2, paste0(EXPORT_PATH, "/household_survey_aim2.RDS"))

saveRDS(loc_visit_aim1, paste0(EXPORT_PATH, "/locations_visited_aim1.RDS"))
saveRDS(loc_visit_aim2, paste0(EXPORT_PATH, "/locations_visited_aim2.RDS"))

saveRDS(exit_interview_aim1, paste0(EXPORT_PATH, "/exit_interview_aim1.RDS"))
saveRDS(exit_interview_aim2, paste0(EXPORT_PATH, "/exit_interview_aim2.RDS"))
