...
# Load packages
pacman::p_load(dplyr, ggplot2, gtsummary, Hmisc, knitr, kableExtra, lubridate, 
               patchwork, readr, stringr,table1, tidyr, openxlsx)
...
source("../../scripts/cleaning/moz_hhid_corrections.R")
# ~/Dropbox/Work-Emory/4.Data and Analysis/GlobalMix/Mozambique/Quantitative/aim_1/globalmix-mozambique-aim-1/scripts/cleaning
...
study_site <- study_site %>%
  mutate(study_site = case_when(study_site == 1 ~ "Rural",
                                study_site == 2 ~ "Urban"),
...
participant_vars <- c("rec_id", "individual_id", "study_site", "aim", "isindex", "sensor_id",
                      "date_enrolled", "age", "months","participant_age", 
...
contact_hh_all <- contact_hh_all %>%
  ...
  select("rec_id","study_site", "hh_membership", "hh_member_hoh", "hh_member_id","hh_member_id2",
         "hh_member_name","hh_member_sex", "hh_member_age", "hh_member_relationship", "hh_member_relationship_other",
...
contact_hh_all <- contact_hh_all%>%
  filter(hh_member_age != 0|is.na(hh_member_age))%>%
  mutate(months = NA)%>%
  bind_rows(contact_age0_months)

rm(age_rural, age_urban, age_combined, contact_age0, contact_age0_months)
```
...
contact_hh_all <- contact_hh_all %>% # 10542
  ...
  dplyr::mutate(contact_age = case_when(as.numeric(hh_member_age) %in% 1:4 ~ "1-4y",
                                        ...
                                        as.numeric(hh_member_age) %in% 40:59 ~ "40-59y",
                                        as.numeric(hh_member_age) >=60 ~ "60+y",
                                        months  %in% 0:5 ~ "<6mo",
                                        months %in% 6:11 ~ "6-11mo",
                                        months %in% 12:48 ~ "1-4y")) %>%
  select(-c(had_contact, share_room, share_bed, cook_food, redcap_event_name,
            hh_member_hoh, hh_member_id, hh_member_id2, hh_member_name, cook_food,
            contact_count, redcap_event_name, repeat_contact_d2,
            hh_member_age, months))
...
contact_nonhh_all <- contact_nonhh_all %>%
  ...
  dplyr::mutate(contact_age = case_when(age_group_contact == 0 ~ "<6mo",
                                              ...
                                              as.numeric(age_contact) %in% 10:19 ~ "10-19y",
                                              as.numeric(age_contact) %in% 20:29 ~ "20-29y",
                                              as.numeric(age_contact) %in% 30:39 ~ "30-39y",
                                              as.numeric(age_contact) %in% 40:59 ~ "40-59y",
                                              as.numeric(age_contact) >=60 ~ "60+y")) %>%
  select(-c(repeat_contact_d2, contact_count, contact_age_yn, contact_id,
...
contact_all <- contact_all %>%
  rename(loc_home = location_contact___0,
         loc_otherhome = location_contact___1,
         loc_school = location_contact___2,
         loc_work = location_contact___3,
         loc_transport = location_contact___4,
         loc_market = location_contact___5,
         loc_street = location_contact___6,
         loc_well  = location_contact___7,
         loc_agricfield  = location_contact___8,
         loc_playground = location_contact___9,
         loc_worship  = location_contact___10,
         loc_other  = location_contact___11)
...
participants_aim1 <- rbind(participants_aim1, participants_aim2_index) %>%
  ...
  select(rec_id, aim, study_site, date_enrolled, participant_age, participant_sex, read_write, 
         enrolled_school, school_level, highest_education, transport_use, occupation, hh_size)%>%
...
contacts_aim1 <- contact_all %>% 
  filter(aim==0)  # 19409 records previous; now 21390; now 27569
...
contacts_aim1 <- contacts_aim1 %>%
  select(rec_id, study_site, survey_date, contact_sex, contact_age, touch_contact, where_contact, 
         contact_mask, duration_contact, frequency_contact, known_contact, starts_with("loc"), 
         hh_membership, hh_member_relationship,study_day, fromdayone, day_of_week, weekday, weeknum, year, weekyear)
# Day1or2 missing


# save participant and contact data for aim 1
# this has been renamed to _aim1b to save the new data after correction in line 1969. MCK
#saveRDS(moz_participant_data_aim1, "../data/clean/moz_participant_data_aim1.RDS")
...
saveRDS(participants_aim2, "../data/clean/moz_participant_data_aim2.RDS")
saveRDS(contacts_aim2, "../data/clean/moz_contact_data_aim2.RDS")
...
saveRDS(hh_survey_aim2, "../data/clean/moz_hh_survey_data_aim2.RDS")
...
exit_interview_aim1 <- exit %>%
  filter(rec_id %in% participants_aim1$rec_id)
saveRDS(exit_interview_aim1, "../../data/clean/moz_exit_interview_data_aim1.RDS")
...
EXPORT_PATH <- "/Users/mck/Library/CloudStorage/OneDrive-EmoryUniversity/3.GlobalMix/Globalmix clean data/Mozambique/"

saveRDS(participants_aim1, paste0(EXPORT_PATH, "aim_1/moz_participant_data_aim1.RDS"))
saveRDS(contacts_aim1, paste0(EXPORT_PATH, "aim_1/moz_contact_data_aim1.RDS"))
saveRDS(hh_survey_aim1, paste0(EXPORT_PATH, "aim_1/moz_household_survey_data_aim1.RDS"))
saveRDS(loc_visit_aim1, paste0(EXPORT_PATH, "aim_1/moz_locations_visited_data_aim1.RDS"))
saveRDS(exit_interview_aim1, paste0(EXPORT_PATH, "aim_1/moz_exit_interview_data_aim1.RDS"))


saveRDS(participants_aim2, paste0(EXPORT_PATH, "aim_2/moz_participant_data_aim2.RDS"))
saveRDS(contacts_aim2, paste0(EXPORT_PATH, "aim_2/moz_contact_data_aim2.RDS"))
saveRDS(hh_survey_aim2, paste0(EXPORT_PATH, "aim_2/moz_household_survey_data_aim2.RDS"))
saveRDS(loc_visit_aim2, paste0(EXPORT_PATH, "aim_2/moz_locations_visited_data_aim2.RDS"))
saveRDS(exit_interview_aim2, paste0(EXPORT_PATH, "aim_2/moz_exit_interview_data_aim2.RDS"))
```