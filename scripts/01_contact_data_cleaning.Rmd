...
# new data downloaded on 07/05/2022

```

```{r raw-data, echo=FALSE}
# import raw data
data <- readRDS("../data/raw/all_data_08092022.RDS")
...
# generate unique number for each participant
...
```{r}
# specify contact day and merge data
...
contact_hh_all <- contact_hh_all %>%
  distinct(rec_id, hh_member_name, Day1or2, .keep_all = TRUE)

# household member data
hh_member = data %>% 
  dplyr::select("rec_id",'redcap_repeat_instrument',hh_member_hoh,
                hh_member_name,hh_member_id,hh_member_sensor,hh_member_sex,
                hh_member_sex_outro,hh_member_age,hh_member_relationship,
  ...
  distinct(rec_id, hh_member_name,.keep_all = TRUE) # 2868 household members recorded
...
cont_d1 = data %>% 
  ...
  dplyr::select("rec_id","individual_id","redcap_event_name","redcap_repeat_instance",
...
cont_d1d2_norep <- cont_d1d2_norep %>%
  select("rec_id","individual_id","survey_date",
...
cont_all <- rbind(cont_d1d2_norep, cont_d2_rep) # cont_d2_rep has no survey_date
# names(cont_d2_rep)
...
```{r}
###!!! I need to know how the age categories for hh_members was done.
...
contact_hh_all$age_group_contact = factor(contact_hh_all$age_group_contact, 
                   levels = c("<6mo", "6-11mo", "1-4y", "5-9y", 
                              ...
                              "40-49y", "50-59y", "60+y")) #, "I don't know")) <- drop this and NA
label(contact_hh_all$age_group_contact) <- "Age group"

contact_hh_all = contact_hh_all %>% 
  dplyr::mutate(contact_id = case_when(contact_id == 0 ~ "No",
                                         contact_id == 1 ~ "Yes",
                                         contact_id == 2 ~ "Not applicable")) # the data dict only says 0, No | 1, Yes, but actually there are 3 levels in this variable 
label(contact_hh_all$contact_id) <- "Did you have contact with this household member?"
...
contact_hh_all$touch_contact = factor(contact_hh_all$touch_contact, 
                   levels = c("Yes", "No", "Do not remember"))
...
contact_hh_all$contact_mask <- factor(contact_hh_all$contact_mask,
                                      levels = c("No, no mask was worn during the encounter",
...
contact_hh_all$known_contact <- factor(contact_hh_all$known_contact,
                                       levels = c("Never met before", "Less than a year",
...
cont_all = cont_all %>% 
  dplyr::mutate(where_contact = case_when(where_contact == 0 ~ "Indoors",
                                          where_contact == 1 ~ "Outdoors",
                                          where_contact == 2 ~ "Both"))
cont_all$where_contact <- factor(cont_all$where_contact,
                                       levels = c("Indoors", "Outdoors", "Both"))
...
cont_all = cont_all %>% 
  dplyr::mutate(contact_mask = case_when(contact_mask == 0 ~ "No, no mask was worn during the encounter",
                                        ...
                                        contact_mask == 2 ~ "Yes, during parts of encounter",
                                        contact_mask == 3 ~ "I don't remember"))
cont_all$cont_mask <- factor(cont_all$contact_mask,
                                   levels = c("No, no mask was worn during the encounter",
                                              "Yes, for the entire encounter",
                                              "Yes, during parts of encounter",
                                              "I don't remember"))
...
cont_all = cont_all %>%
  dplyr::mutate(duration_contact = case_when(duration_contact == 0 ~ "< 5 mins",
                                               ...
                                               duration_contact == 4 ~ "1-4 hrs",
                                               duration_contact == 5 ~ ">4 hrs"))
cont_all$duration_cont <- factor(cont_all$duration_contact,
                                       levels = c("< 5 mins", "5-15 mins", 
                                                  "16-30 mins", "31 mins - 1 hr",
                                                  "1-4 hrs", ">4 hrs"))
...
cont_all = cont_all %>%
  dplyr::mutate(frequency_contact = case_when(frequency_contact == 0 ~ "Never met before",
                                               ...
                                               frequency_contact == 5 ~ "Once per month",
                                               frequency_contact == 6 ~ "Once every 3 months"))
cont_all$frequency_contact <- factor(cont_all$frequency_contact,
                                        levels = c("Never met before", "Rarely",
                                                   "Daily or almost daily","1-3 times per week",
                                                   "Once every 2 weeks", "Once per month",
                                                   "Once every 3 months"))
...
cont_all = cont_all %>%
  dplyr::mutate(known_contact = case_when(known_contact == 0 ~ "Never met before",
                                               ...
                                               known_contact == 4 ~ "6-10 years",
                                               known_contact == 5 ~ "More than 10 years"))
cont_all$known_contact <- factor(cont_all$known_contact,
                                    levels = c("Never met before", "Less than a year",
                                               "1-2 years", "3-5 years", "6-10 years",
                                               "More than 10 years"))
...
cont_all = cont_all %>% 
  dplyr::mutate(age_group_contact = case_when(age_group_contact == 0 ~ "<6mo",
                                               ...
                                               age_group_contact == 7 ~ "40-49y",
                                               age_group_contact == 8 ~ "50-59y",
...
```{r}
# Merge household and hon-household contacts for Manhica only
contact_hh_rural <- contact_hh_all[which(contact_hh_all$study_site=="Manhiça"),]
contact_nonhh_rural <- cont_all[which(cont_all$study_site=="Manhiça"),]
...
keep_vars <- c("rec_id", "survey_date", "age_group_contact", "contact_sex","touch_contact", "fromdayone",
...
contact_nonhh_rural <- contact_nonhh_rural %>%
  select(any_of(keep_vars))
...
intersect_ids <- intersect(unique(contact_hh_rural$rec_id),
                           unique(contact_nonhh_rural$rec_id))
contact_hh_rural <- contact_hh_rural[contact_hh_rural$rec_id %in% intersect_ids,]
contact_nonhh_rural <- contact_nonhh_rural[contact_nonhh_rural$rec_id %in% intersect_ids,]


contact_rural_all <- rbind(contact_hh_rural, contact_nonhh_rural)
```
...
contact_hh_summary <- contact_hh_rural %>% 
  select("contact_sex","age_group_contact","relationship","fromdayone",
...
contact_summary <- contact_nonhh_rural %>%
  select("contact_sex","age_group_contact","relationship","fromdayone",
         "touch_contact","where_contact","contact_mask","duration_contact",
         "frequency_contact","known_contact","location_contact___0",
         "location_contact___1","location_contact___2","location_contact___3",
...
ind_summary <- ind %>%
  ...
  select("sex", "sex_other", "birthdate","age_cat", "read_write",
         "enrolled_school", "highest_educ", "school_level", "occupation", 
         "transport_use","hh_resp_exposure", "child_breastfeed", "num_sibling",
         "child") %>%
  tbl_summary(percent="column",
              digits = all_categorical() ~ 1) %>%
  bold_labels() %>%
...
ind <- ind %>%
  select("rec_id","date_enrolled","aim", "study_site", "sex", "sex_other",
         "birthdate","age", "age_cat","read_write", "enrolled_school", 
         "highest_educ", "school_level", "occupation", 
         "transport_use","hh_resp_exposure", "child_breastfeed", "num_sibling",
         "child", "sensor_id")


# now commented out because the data is not correct.

# saveRDS(ind, "../data/individual_info.RDS")
# saveRDS(ind_contact_hh, "../data/ind_contact_hh.RDS")
# saveRDS(ind_contact, "../data/ind_contact_nonhh.RDS")
# 
# saveRDS(contact_rural_all, "../data/contact_rural_all.RDS")
```
...
hh_survey = hh_survey %>% 
  dplyr::mutate(study_site = case_when(study_site == 1 ~ "Manhiça",
                ...
                hh_roof_material = case_when(hh_roof_material == 0 ~ 'Natural cover (Ex. Straw, palm leaf or grass)',
                                            hh_roof_material == 1 ~ 'Rudimentary covering (eg rustic rug, rustic wooden boards or cardboard)',
                                            hh_roof_material == 2 ~ 'Modern roofing (eg zinc sheet, concrete, ceramic, tiles)',
                                            hh_roof_material == 3 ~ 'Other'),
                hh_wall_material = case_when(hh_wall_material == 0 ~ 'Natural walls (Ex. No wall, reeds, palm leaves, stakes)',
                                            hh_wall_material == 1 ~ 'Rudimentary walls (eg Bamboo, pressed wood, adobe, adobe-matted stone, recycled wood, adobe-matted pegs, unburned bricks, tin, cardboard)',
                                            hh_wall_material == 2 ~ 'Modern walls (eg Cement, limestone/cement, bricks, cement blocks, cement-covered adobe, wooden planks/laths, burnt bricks, zinc sheet walls)',
                                            hh_wall_material == 3 ~ 'Other'),
                hh_ac = case_when(hh_ac == 0 ~ 'No',
                                  hh_ac == 1 ~ 'Yes',
                                  hh_ac == 2 ~ "I don't know"),
                hh_fan = case_when(hh_fan == 0 ~ 'No',
                                  hh_fan == 1 ~ 'Yes',
                                  hh_fan == 2 ~ "I don't know")) 
...
label(hh_survey$hh_transport___4) <- "None of the above"
...
label(hh_survey$animal_touch___8) <- "Contact with none of the above"
...
label(hh_survey$animal_injury___8) <- "Injury from none of the above"
...
label(hh_survey$animal_cook___8) <- "Cooked none of the above"
...
label(hh_survey$water_purify_method___8) <- "I don't know/none of the above"
...
hh_survey = hh_survey %>% 
  ...
  dplyr::mutate(wash_hands_defecatelb = case_when(wash_hands_defecate == 0 ~ "Never",
                                         ...
                                         wash_hands_defecate == 4 ~ "Not applicable (does not help a child defecate)"),
...
label(hh_survey$wash_hands_foodlb) <- "Do members of your household wash their hands before touching food (Ex. cooking, preparing, before eating)?"
...
hh_survey = hh_survey %>% 
  ...
  dplyr::mutate(hh_smokinglb = case_when(hh_smoking == 0 ~ "No",
                                         hh_smoking == 1 ~ "Yes",
                                         hh_smoking == 2 ~ "I don't know"),
                #Do you cook food inside or outside the house?
                hh_cooklb = case_when(hh_cook == 0 ~ "Inside",
                                         hh_cook == 1 ~ "Outside",
                                         hh_cook == 2 ~ "Both"))
label(hh_survey$hh_smokinglb) <- "Does anyone in your household (including yourself) smoke tobacco products?"
label(hh_survey$hh_cooklb) <- "Do you cook food inside or outside the house?"
...
# saveRDS(hh_survey, "../Data/household_survey.RDS")
```

```{r}
#create the date for participants' location visit information in 2 days 
loc_visit_d1 <- data %>%
  dplyr::filter(redcap_repeat_instrument == "dirio_de_locais_visitados_dia_1") %>%
  select("rec_id","individual_id","redcap_event_name","redcap_repeat_instrument","redcap_repeat_instance","place_visited_d1","place_visited_d1_other","num_pax_place_d1","time_visited_d1","visited_frequency_d1","dirio_de_locais_visitados_dia_1_complete")
...
label(loc_visit_d1$time_visited_d1) <- "How long did you spend in this place?"
...
loc_visit_d2 <- data %>%
  ...
  select("rec_id","individual_id","redcap_event_name","redcap_repeat_instrument","redcap_repeat_instance","place_visited_d2","place_visited_d2_other","num_pax_place_d2","time_visited_d2","visited_frequency_d2","dirio_de_locais_visitados_dia_2_complete")
...
```{r}
#combine data for location visit day1 and day2 together
names(loc_visit_d1)[grep("_d1", names(loc_visit_d1))] <-  gsub("_d1", "", names(loc_visit_d1)[grep("_d1", names(loc_visit_d1))])

names(loc_visit_d1)[11] <- "survey_complete"

names(loc_visit_d2)[grep("_d2", names(loc_visit_d2))] <-  gsub("_d2", "", names(loc_visit_d2)[grep("_d2", names(loc_visit_d2))])

names(loc_visit_d2)[11] <- "survey_complete"

loc_visit <- rbind(loc_visit_d1, loc_visit_d2)

ind_loc_visit <- loc_visit %>%
  left_join(ind, by='rec_id')

saveRDS(ind_loc_visit, "../Data/ind_location_visit.RDS")
```
...
saveRDS(ind_exit, "../Data/ind_exit.RDS")
```


```{r}
# add index participants from aim2 to aim1 data for analysis
ind_contact_aim1 <- ind_contact %>% 
  filter(aim==1)

ind_contact_aim2_index <- ind_contact %>% 
  filter(aim==2, age_cat %in% c("<6mo","6-11mo", "1-4y"))

ind_contact_aim1_final <- rbind(ind_contact_aim1, ind_contact_aim2_index)
saveRDS(ind_contact_aim1_final, "../data/ind_contact_aim1_final.RDS")



ind_contact_hh_aim1 <- ind_contact_hh %>% 
  filter(aim==1) 

ind_contact_hh_aim2_index <- ind_contact_hh %>% 
  filter(aim==2, age_cat %in% c("<6mo","6-11mo", "1-4y"))
ind_contact_hh_aim1_final <- rbind(ind_contact_hh_aim1, ind_contact_hh_aim2_index)
saveRDS(ind_contact_hh_aim1_final, "../data/ind_contact_hh_aim1_final.RDS")
```
