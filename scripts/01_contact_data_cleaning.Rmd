...
ind$study_site <- factor(ind$study_site, levels = c("Manhiça", "Polana Caniço"),
...
ind$sex <- factor(ind$sex, levels = c("Female", "Male"))

# literacy 
ind <- ind %>% 
  dplyr::mutate(read_write = case_when(read_write == 0 ~ "No", 
                                         read_write == 1 ~ "Yes"))
ind$read_write <- factor(ind$read_write, 
                         levels = c("Yes", "No"),
                         labels = c("Yes", "No"))
...
ind$enrolled_school <- factor(ind$enrolled_school, 
                   levels = c("Yes", "No"),
                   labels = c("Yes", "No"))
...
ind <- ind %>% 
  dplyr::mutate(school_level = case_when(school_level == 0 ~ "None", 
                                           ...
                                           school_level == 6 ~ "Bachelors or more")) # More than Bachelors
ind$school_level <- factor(ind$school_level, 
...
ind <- ind %>% 
  dplyr::mutate(highest_educ = case_when(highest_educ == 0 ~ "None", 
                                           ...
                                           highest_educ == 4 ~ "College/University", # 1-3 years of college/university
                                           highest_educ == 5 ~ "Bachelors or more", # Bachelors
                                           highest_educ == 6 ~ "Bachelors or more")) # More than Bachelors
ind$highest_educ <- factor(ind$highest_educ, 
...
ind$occupation <- factor(ind$occupation, 
                   levels = c("Unemployed", "Student", "Homemaker/Housewife",
                              "Casual laboror", "Farmer", "Fishing", 
                              "Business person", "Office worker", "Retired",
                              "Other"),
                   labels = c("Unemployed", "Student", "Homemaker",
                              "Casual laboror", "Farmer", "Fishing", 
                              "Business person", "Office worker", "Retired",
                              "Other"))
...
ind$transport_use <- factor(ind$transport_use, 
                            levels = c("Never", "Rarely", "Daily or almost daily",
...
ind$hh_resp_exposure <- factor(ind$hh_resp_exposure, levels = c("Yes", "No", 
                                                                "Unknown"))
...
ind$child <- factor(ind$child, levels = c("Yes", "No", "Not applicable"))
...
ind$child_breastfeed<- factor(ind$child_breastfeed, levels = c("Yes", "No", 
                                                                "Not applicable"))
...
exit <- exit %>% 
  dplyr::mutate(study_site = case_when(study_site == 1 ~ "Manhiça",
                                         study_site == 2 ~ "Polana Caniço"))
exit$study_site <- factor(exit$study_site, levels = c("Manhiça", "Polana Caniço"),
                          labels = c("Rural", "Urban"))
exit <- exit %>% 
  dplyr::mutate(aim = case_when(aim == 0 ~ 1,
                                 aim == 1 ~ 2)) 
...
exit <- exit %>% 
  dplyr::mutate(behavior_changelb = case_when(behavior_change == 1 ~ "No change", 
                                              ...
                                              behavior_change == 2 ~ "Increased", # My interactions have increased
                                              behavior_change == 3 ~ "Reduced", # My interactions have reduced
                                              behavior_change == 4 ~ "Unknown"))
exit$behavior_changelb <- factor(exit$behavior_changelb, levels = c("Increased", "No change",  
                                                                    "Reduced", "Unknown"))
...
exit <- exit %>% 
  dplyr::mutate(movement_changelb = case_when(movement_change == 1 ~ "No change",
                                              ...
                                              movement_change == 2 ~ "Increased",
                                              movement_change == 3 ~ "Reduced",
                                              movement_change == 4 ~ "Unknown"))
...
exit <- exit %>% 
  dplyr::mutate(days_left_homelb = case_when(days_left_home == 0 ~ "0",
                                           ...
                                           days_left_home == 3 ~ ">7",
                                           days_left_home == 4 ~ "Unknown",
                                           days_left_home == 5 ~ "Unknown")) #No response
...
contact_hh_all <- contact_hh_all %>%
  distinct(rec_id, hh_member_id2, study_day, .keep_all = TRUE) # n=15445, hh_member_id, 
...
contact_vars <- c("rec_id", "study_site", "survey_date", "study_day", "age_group_contact", 
...
# factor vars
contact_all$contact_sex <- factor(contact_all$contact_sex , levels = c("Female", "Male"))

contact_all$hh_member_relationship <- factor(contact_all$hh_membership,
...
hh_survey <- hh_survey %>% 
  dplyr::mutate(study_site = case_when(study_site == 1 ~ "Manhiça",
                ...
                hh_ac = case_when(hh_ac == 0 ~ 'No',
                                  hh_ac == 1 ~ 'Yes',
                                  hh_ac == 2 ~ "Unknown"),
                factor(hh_ac, levels = c("Yes", "No", "Unknown")),
                
                hh_fan = case_when(hh_fan == 0 ~ 'No',
                                  hh_fan == 1 ~ 'Yes',
                                  hh_fan == 2 ~ "Unknown"),
                factor(hh_fan, levels = c("Yes", "No", "Unknown")))
...
hh_survey = hh_survey %>% 
  ...
  dplyr::mutate(hh_smokinglb = case_when(hh_smoking == 0 ~ "No",
                ...
                factor(hh_smokinglb, levels = c("Yes", "No", "Unknown")),
...
```{r data-export-csv, echo=FALSE}
EXPORT_PATH <- "/Users/mck/Library/CloudStorage/OneDrive-EmoryUniversity/4.Data and Analysis/GlobalMix/globalmix-clean-data/Mozambique/aim_1/data"
