...
aim2 <- readRDS("../data/clean/participant_data_aim2.RDS") %>%
  select("rec_id", "date_enrolled", "isindex", "aim", "study_site", "sensor_id", 
...
rural_sensors <- rural_sensors %>%
  ...
  distinct(rec_id, .keep_all = TRUE) %>%
  select("rec_id", "date_enrolled", "isindex", "aim", "study_site", "sensor_id", "hwid",
         "participant_sex", "sex_other", "age", "participant_age", "read_write", 
         "occupation", "occupation_other", "enrolled_school", "highest_educ",  
         "school_level", "transport_use", "hh_resp_exposure", "child", 
         "child_breastfeed", "num_sibling")
...
write.xlsx(rural_missing_sensorid, 
           file =  "../data/sensor_metadata/mozambique_missing_aim2_data.xlsx",
           sheetName = "rural", 
