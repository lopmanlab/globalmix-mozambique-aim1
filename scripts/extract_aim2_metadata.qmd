...
cat("There are still some households with only 1 member.")
...
# for now, we drop these 9 records that have sensor_id==0
...
urban_missing_recid <- c(909, 1486, 1111, 1109, 1110, 1493, 1494, 1495, 
                         1510, 1511, 1631)

cat("visual check reveals", length(urban_missing_recid), "missing sensor ids (7) or missing hwid (2). So, we drop these.")
...
urban_hhsize_2 <- n_urban_household_members %>%
  filter(n < 3)
cat("There are", nrow(urban_hhsize_2), "households with ≤2 members. This should 
...
# How many individuals per household? 
cat("There are 10 households with ≤2 members, 6 with only 1 member.")
