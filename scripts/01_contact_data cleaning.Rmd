...
hh_member = data %>% 
  ...
  distinct(rec_id, hh_member_name,.keep_all = TRUE)
...
ind <- data %>% 
  ...
  dplyr::select("rec_id", "date_enrolled", "sex", "sex_other", "birthdate","age", 
                ...
                "occupation___3", "occupation___4", "occupation___5", "occupation___6", "occupation___7", "occupation___8", "occupation___9", "occupation_other", "enrolled_school", "highest_educ", "school_level", "transport_use", "hh_resp_exposure", "child_breastfeed", "num_simbling", "inscrio_complete", "sensor_id","child", "redcap_repeat_instrument") %>%
    dplyr::filter(is.na(redcap_repeat_instrument)) %>%
  left_join(study_site, by="rec_id")
...
ind = ind %>% 
  dplyr::mutate(highest_educ = case_when(highest_educ == 0 ~ "No schooling", 
                                           ...
                                           highest_educ == 6 ~ "More than Bachelors")) ## check spelling, initially "Bachelors"
...
ind_contact_aim1 <- ind_contact %>% filter(aim==1) 
ind_contact_aim2_index <- ind_contact %>% filter(aim==2, age_cat %in% c("<6mo","6-11mo", "1-4y"))
...
ind_contact_hh_aim1 <- ind_contact_hh %>% filter(aim==1) 
ind_contact_hh_aim2_index <- ind_contact_hh %>% filter(aim==2, age_cat %in% c("<6mo","6-11mo", "1-4y"))
