...
table2 <- contacts %>%
  dplyr::select(study_site, contact_age, contact_sex, hh_membership, 
                hh_member_relationship, contact_mask, known_contact, 
                duration_contact, frequency_contact, where_contact) %>%
  tbl_summary(by = study_site,
              percent = "col",
              digits = all_categorical() ~ 0,
              missing = "ifany") %>%
...
for (i in 1:nrow(variables)){
  ...
  n <- length(unique(df_contact_rural[rec_id[i]]))
  variables$var[[i]]
  print(n)
}
