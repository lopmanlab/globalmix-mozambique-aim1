...
This analysis focuses on aim 1 only. Data were collected from Manhica (rural site) and Polana Canhico (urban site) between April 2020 and April 2021. Individuals were identified from the health and demographic surveillance system available in each site. We aimed to recruit 63 individuals from each of 10 age groups (\<6 months, 6-11 months, 1-4 years, 5-9 years, 10-14, 15-19, 20-29, 30-39, 40-59 and 60+ years).
...
@tbl-participant-summary below presents the characteristics of the participants in the rural and urban site. A total of `r nrow(participants)` individuals participated in the study, with `r N_rural` and `r N_urban` participants recruited from the rural and urban sites, respectively. In some age groups (\<5 yrs and 40-59 yrs) we recruited more than the target. By site, there was no difference in recruitment by age, sex, and school enrollment status.
...
table1 <- participants %>%
  # dplyr::filter(aim==1) %>%
  dplyr::select(study_site, participant_sex, participant_age,  read_write, enrolled_school, 
...
@tbl-contact-characteristics shows the characteristics of the number of contacts per site. Overall, `r tot_contacts` contacts were reported with slightly above half (`r tot_rural_contacts`, `r round(100*(tot_rural_contacts/tot_contacts), 0)`%) from the rural site. In each site, 6% of the total contacts were reported with children aged \<5 years (7% rural, 6% urban), with only 1% of the contacts in each site happening with infants aged \<6 months (148 rural, 82 urban).
...
table2 <- contacts %>%
  dplyr::select(study_site, contact_age, contact_sex, hh_membership, contact_mask, 
...
```
@fig-contact-distribution-overall shows the distribution of all reported contacts over the two days. 
...
hist_rural <- contacts_site %>%
  ...
  add_annotations(x = mean_contacts_site$mean[1], 
                  y = 80,
                  text = paste(mean_contacts_site$mean[1], " (95% CI ", ci_contacts_rural$`2.5 %`,"-",ci_contacts_rural$`97.5 %`,")", sep=""), 
...
hist_urban <- contacts_site %>%
  ...
  add_annotations(x = mean_contacts_site$mean[2], 
                  y = 90,
                  text = paste(mean_contacts_site$mean[2], " (95% CI ", ci_contacts_urban$`2.5 %`,"-",ci_contacts_urban$`97.5 %`,")", sep=""), 
                  showarrow = F, xanchor = "left") %>%
  layout(xaxis = list(title = "Num of contacts")) %>%
...
variables <- data.frame(var=c("participant_sex","participant_age", "hh_member_relationship", 
                              ...
                              "known_contact", "contact_mask2"),
                        name=c("Sex", "Age", "Relationship to household member", 
...
At face value, there is no difference between contacts reported on day 1 compared to day 2. Additional analysis can include: 1. mean number of contacts over both days 2. mean number of contacts of unique contacts only over both days 3. mean number of unique contacts reported on both day 1 and 2
...
#| fig-cap: "Histogram of contact distribution by site showing mean contact rate over two days (vertical line)"
#| warning: false
...
hist_rural_d1 <- contacts_site_d1 %>%
  ...
  plotly::plot_ly(x=~num_contacts, colors="#a6611a") %>%
  add_lines(y = range(0:100),
            x = mean_contacts_site_d1$mean[1], #,mean(contacts_site$num_contacts),
            line = list(color = "black"),
            showlegend = FALSE) %>%
  add_annotations(x = mean_contacts_site_d1$mean[1], 
                  y = 90,
                  text = paste(mean_contacts_site_d1$mean[1], " (95% CI ", ci_contacts_urban_d1$`2.5 %`,"-",ci_contacts_urban_d1$`97.5 %`,")", sep=""), 
                  showarrow = F, xanchor = "left") %>%
  layout(annotations = title_rural_d1,
...
hist_urban_d1 <- contacts_site_d1 %>%
  ...
  plotly::plot_ly(x=~num_contacts, colors="#018571") %>%
  add_lines(y = range(0:100),
            x = mean_contacts_site_d1$mean[2], #,mean(contacts_site$num_contacts),
            line = list(color = "black"),
            showlegend = FALSE) %>%
  add_annotations(x = mean_contacts_site_d1$mean[2], 
                  y = 90,
                  text = paste(mean_contacts_site_d1$mean[2], " (95% CI ", ci_contacts_urban_d1$`2.5 %`,"-",ci_contacts_urban_d1$`97.5 %`,")", sep=""), 
                  showarrow = F, xanchor = "left") %>%
  layout(annotations = title_urban_d1,
         xaxis = list(title = "Num of contacts")) %>%
  add_histogram(name="Urban")

hist_d1 <- subplot(hist_rural_d1, hist_urban_d1, nrows=2, shareX = TRUE)
...
contacts_education <- df_contact_d1 %>%
  dplyr::group_by(rec_id, study_site, enrolled_school) %>%   # group by id and count number of contacts
...
contacts_weekday <- df_contact_d1 %>%
  dplyr::group_by(rec_id, study_site, weekday) %>%   # group by id and count number of contacts
  dplyr::summarize(num_contacts = n()) %>%
  na.omit()

# 6. year of the week
contacts_weekyear <- df_contact_d1 %>%
  dplyr::group_by(rec_id, study_site, weekyear) %>%   # group by id and count number of contacts
  dplyr::summarize(num_contacts = n())

# 7. household vs nonhousehold member
contacts_hhmember <- df_contact_d1 %>%
  dplyr::group_by(rec_id, study_site, participant_age, participant_sex, hh_membership) %>%   
...
# overall by household membership
fig5_age_box <- contacts_age %>%
  plotly::plot_ly(x=~participant_age,  y=~num_contacts,  color=~study_site,  
                  type = 'box',  colors = c("#a6611a", "#018571")) %>%
  plotly::layout(xaxis = list(title = "Participant age"),  
...
### Overall distribution of contacts by rural and urban site

```{r}
#| label: fig-contact-boxplot
#| fig-cap: "Overall distribution of contacts by rural and urban site"
#| warning: false
fig5_age_box
```

### Distribution of contacts by current school enrollment

```{r}
#| label: fig-contact-boxplot-education
#| fig-cap: "Distribution of contacts by current school enrollment"
#| warning: false
fig7_education_box
```

### Distribution of contacts by day of the week
...
```{r}
#| label: fig-contact-boxplot-weekend
#| fig-cap: "Distribution of contacts by weekday/ weekend"
#| warning: false
fig9_weekend_box
```

### Distribution of contacts by sex
...
#| warning: false
# sex
fig6_sex_rural_box <- contacts_sex %>%
  dplyr::filter(study_site == "Rural") %>%
...
fig6_sex_urban_box <- contacts_sex %>%
  dplyr::filter(study_site == "Urban") %>%
  plotly::plot_ly(x=~participant_age,  y=~num_contacts,  color=~participant_sex,  
                      type = 'box',  colors = c("#ef8a62", "#67a9cf")) %>%
...
subplot(style(fig6_sex_rural_box, showlegend = F), nrows=2, fig6_sex_urban_box, shareX = TRUE)
```
...
```{r}
#| label: fig-contact-matrix-rural
#| fig-cap: ""
...
```{r}
#| label: fig-contact-matrix-urban
#| fig-cap: ""
...
# contact_behav_avg <- df_contact_d1 %>% 
#   dplyr::select(study_site, touch_contact, where_contact, contact_mask, duration_contact) %>%
#   mutate(where_contact = as.character(where_contact),
#          touch_contact = as.character(touch_contact),
#          contact_mask = as.character(contact_mask),
#          duration_contact = as.character(duration_contact)) %>%
#   pivot_longer(cols = touch_contact:duration_contact, names_to="var", values_to="value") %>%
#   dplyr::group_by(study_site, var, value) %>%
#   dplyr::summarize(n=n()) %>%
#   na.omit() %>%
#   left_join(df_contact_d1 %>% 
#               dplyr::group_by(study_site) %>%
#               dplyr::summarize(tot_contacts=n()), by = c("study_site" = "study_site")) %>%
#   mutate(prop = n/tot_contacts,
#          participant_age = "All")
# 
# dat2 <- contact_behav %>%
#   mutate(var = as.character(participant_age)) %>%
#   bind_rows(contact_behav_avg) %>%
#   mutate(participant_age = factor(participant_age, levels = c("All", "<6mo", "6-11mo", 
#                                                               "1-4y", "5-9y", "10-14y", 
#                                                               "15-19y", "20-29y", 
#                                                               "30-39y", "40-59y", " 60+y")))
