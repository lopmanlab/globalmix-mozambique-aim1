...
hh = hh %>% 
  dplyr::mutate(water_purifylb = case_when(water_purify == 0 ~ "No",
                                         water_purify == 1 ~ "Yes",
                                         water_purify == 2 ~ "I don't know"))
label(hh$water_purifylb) <- "Does your household do anything to the water to make it safer to drink?"
temp10 <- c("water_purifylb","water_purify_method___1","water_purify_method___2","water_purify_method___3",
...
temp11 <- c("wash_hands_defecatelb","wash_hands_foodlb","wash_hands_toiletlb")
...
label(hh$wash_hands_defecatelb) <- "Do you wash your hands after helping a child defecate?"
label(hh$wash_hands_foodlb) <- "Do members of your household wash their hands before touching food (Ex. cooking, preparing, before eating)?"
label(hh$wash_hands_toiletlb) <- "Do members of your household wash their hands after using the toilet?"
t5 <- hh %>%
  select(all_of(c(temp11))) %>%
   tbl_summary(percent="column",
              digits = all_categorical() ~ 1,
              missing="no") %>%
  bold_labels() %>%
  modify_header(label = "**Variable**") %>%
  modify_caption("**Table 5: Wash hand (Row %).**")
t5
```

```{r toilet, echo=FALSE}
temp12 <- c("hh_toiletlb","hh_toilet_locationlb","child_stool_disposallb")
hh = hh %>% 
  #What kind of toilet facility do members of your household usually use?
  dplyr::mutate(hh_toiletlb = case_when(hh_toilet == 0 ~ "Flush toilet",
                                         hh_toilet == 1 ~ "Toilet without flush",
                                         hh_toilet == 2 ~ "Improved latrine",
                                         hh_toilet == 3 ~ "Improved traditional latrine",
                                         hh_toilet == 4 ~ "Unimproved latrine",
                                         hh_toilet == 5 ~ "Without"),
                #Where is this toilet facility located?
                hh_toilet_locationlb = case_when(hh_toilet_location == 0 ~ "Inside house",
                                         hh_toilet_location == 1 ~ "Outside house but inside compound",
                                         hh_toilet_location == 2 ~ "Elsewhere"),
                #The last time the youngest child in your household passed stool, what was done to dispose of the stool?
                child_stool_disposallb = case_when(child_stool_disposal == 0 ~ "Child used toilet",
                                         child_stool_disposal == 1 ~ "Put into toilet or latrine",
                                         child_stool_disposal == 2 ~ "Put into drain or ditch",
                                         child_stool_disposal == 3 ~ "Thrown into garbage",
                                         child_stool_disposal == 4 ~ "Buried",
                                         child_stool_disposal == 5 ~ "Left it in the open",
                                         child_stool_disposal == 6 ~ "Other",
                                         child_stool_disposal == 7 ~ "I don't know",
                                         child_stool_disposal == 8 ~ "Does not apply"))
label(hh$hh_toiletlb) <- "What kind of toilet facility do members of your household usually use?"
label(hh$hh_toilet_locationlb) <- "Where is this toilet facility located?"
label(hh$child_stool_disposallb) <- "The last time the youngest child in your household passed stool, what was done to dispose of the stool?"

t6 <- hh %>%
  select(all_of(c(temp12))) %>%
   tbl_summary(percent="column",
              digits = all_categorical() ~ 1,
              missing="no") %>%
  bold_labels() %>%
  modify_header(label = "**Variable**") %>%
  modify_caption("**Table 6: Toilet (Row %).**")
t6

```

```{r smoke_exposure, echo=FALSE}
temp13 <- c("hh_smokinglb","hh_cooklb", "hh_fuel___0","hh_fuel___1",
            "hh_fuel___2","hh_fuel___3")
...
hh = hh %>% 
  ...
  dplyr::mutate(hh_smokinglb = case_when(hh_smoking == 0 ~ "No",
                ...
                hh_cooklb = case_when(hh_cook == 0 ~ "Inside",
                                         hh_cook == 1 ~ "Outside",
                                         hh_cook == 2 ~ "Both"))
label(hh$hh_smokinglb) <- "Does anyone in your household (including yourself) smoke tobacco products?"
label(hh$hh_cooklb) <- "Do you cook food inside or outside the house?"

#What do you use to cook food? 
label(hh$hh_fuel___0) <- "Biomass (Wood, Coal, Manure)"
label(hh$hh_fuel___1) <- "Gas"
label(hh$hh_fuel___2) <- "Electricity"
label(hh$hh_fuel___3) <- "Other"


t7 <- hh %>%
  select(all_of(c(temp13))) %>%
   tbl_summary(percent="column",
              digits = all_categorical() ~ 1,
              missing="no") %>%
  bold_labels() %>%
  modify_header(label = "**Variable**") %>%
  modify_caption("**Table 7: Smoking, Cook and Fuel (Row %).**")
t7
```

```{r resp_illness, echo=FALSE}
temp14 <- c("hh_asthmalb","hh_tblb")
hh = hh %>% 
...
label(hh$hh_asthmalb) -> "Does anyone in your household have asthma?"
label(hh$hh_tblb) -> "Has anyone in your household been diagnosed with Tuberculosis (TB) in the past year?"

t8 <- hh %>%
  select(all_of(c(temp14))) %>%
   tbl_summary(percent="column",
              digits = all_categorical() ~ 1,
              missing="no") %>%
  bold_labels() %>%
  modify_header(label = "**Variable**") %>%
  modify_caption("**Table 8: Respiratory Disease (Row %).**")
t8
...
```{r}
hh_member = data %>% 
  dplyr::select("rec_id","individual_id","redcap_event_name","redcap_repeat_instrument","redcap_repeat_instance",hh_member_hoh,hh_member_name,hh_member_id,hh_member_sensor,hh_member_sex,hh_member_sex_outro,hh_member_age,hh_member_relationship,hh_member_relationship_other,share_room,share_bed,cook_food,hh_member_hhform,hh_member_instance,informaes_de_agregado_familiar_e_casa_complete) %>% 
  dplyr::filter(redcap_repeat_instrument == "informaes_sobre_os_membros_da_famlia")
```

```{r}
hh_member = hh_member %>% 
  dplyr::mutate(hh_member_hohlb = case_when(hh_member_hoh == 0 ~ "No",
                                         hh_member_hoh == 1 ~ "Yes"))
label(hh_member$hh_member_hohlb) <- "Are you interviewing the head of household?"

hh_member = hh_member %>% 
  dplyr::mutate(hh_member_sexlb = case_when(hh_member_sex == 0 ~ "Female",
                                         hh_member_sex == 1 ~ "Male",
                                         hh_member_sex == 2 ~ "Transsexual Female",
                                         hh_member_sex == 3 ~ "Transsexual Male",
                                         hh_member_sex == 4 ~ "Other"))
label(hh_member$hh_member_sexlb) <- "What is the sex of this household member?"

hh_member = hh_member %>% 
  dplyr::mutate(hh_member_relationshiplb = case_when(hh_member_relationship == 0 ~ "Spouse",
                                        ...
                                        hh_member_relationship == 7 ~ "Niece/Nephew",
                                        hh_member_relationship == 8 ~ "Other Relative",
                                        hh_member_relationship == 9 ~ "Not Related",
                                        hh_member_relationship == 10 ~ "Not applicable (This is the Head of Household)")) 
label(hh_member$hh_member_relationshiplb) <- "How is this member related to you?"

hh_member = hh_member %>% 
  dplyr::mutate(share_roomlb = case_when(share_room == 0 ~ "No",
                                         share_room == 1 ~ "Yes"))
label(hh_member$share_roomlb) <- "Does this household member share a room to sleep with anyone?"

hh_member = hh_member %>% 
  dplyr::mutate(share_bedlb = case_when(share_bed == 0 ~ "No",
                                         share_bed == 1 ~ "Yes"))
label(hh_member$share_bedlb) <- "Does this household member share the same bed with anyone?"

hh_member = hh_member %>% 
  dplyr::mutate(cook_foodlb = case_when(cook_food == 0 ~ "No",
                                         cook_food == 1 ~ "Yes"))
label(hh_member$cook_foodlb) <- "Does this person cook food for the household?"
