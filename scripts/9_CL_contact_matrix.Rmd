...
ind <- ind %>% filter(
  aim ==1    ## retain 1328 from aim 1
) %>% filter(
  rec_id %in% unlist(ind_exit$rec_id) ##retain 1270 with exit survey
)
...
```{r}
ind <- ind %>%
    filter(!rec_id %in% unlist(missing_hh_nonhh$rec_id)) ## Dropped an additional 28 without any diaries (but had exit interviews, all from polana canico)
```


Check those without household contact but have other contacts live alone
```{r}

live_alone <- hh_survey %>% filter(rec_id %in% unlist(ind$rec_id)) %>%
              filter(hh_occupants==1) 
##One person who lived alone apparently reported hh contacts, probably fine
missing_hh %>% filter(
  rec_id %in% unlist(live_alone$rec_id)
)

##15 people had household members but no reported household contacts
## Retain these for now
missing_hh%>% filter(
  !rec_id %in% unlist(missing_hh_nonhh$rec_id)
) %>%
  filter(
    !rec_id %in% live_alone$rec_id
  )

```

```{r}
## Note that self refers to HoH not the participant, it looks like the entry for the partiicpant has already been removed from the HH contacts
contact_hh %>% select(rec_id, individual_id, contact_id:study_site)

##Combine HH and nonhh contact and figure out the day/dates


contact_all <- plyr::rbind.fill(
  contact_nonhh %>% select(rec_id,survey_date, study_site, Day1or2, contact_age_yn, age_contact, age_group_contact, touch_contact, where_contact, contact_mask, duration_contact, location_contact___0:location_contact___11)%>%
  arrange(rec_id, Day1or2),

contact_hh %>% select(rec_id, survey_date,study_site, Day1or2, hh_member_age, age_group_contact, touch_contact, where_contact, contact_mask, duration_contact, location_contact___0:location_contact___11, hh_member_relationship)%>%
  rename(age_contact = hh_member_age))

## Get age of the participant
contact_all <- contact_all %>%
                left_join(ind%>%select(rec_id, age,age_cat), by = c("rec_id"="rec_id"))%>%
                rename(age_part =age)
...
contact_all <- contact_all %>%
              mutate(cnt_home = ifelse(location_contact___0==1,1,0),
                     ...
                     cnt_otherplace = ifelse(location_contact___1==1|location_contact___4==1|location_contact___5==1|location_contact___6==1|location_contact___7==1|location_contact___8==1|location_contact___9==1|location_contact___10==1|location_contact___11==1,1,0))

## recat relationship
contact_all <- contact_all %>%
                mutate(
                  rel = case_when(
                    is.na(hh_member_relationship) ~"non_hh",
...
t1 <- ind %>%
  ...
  select(sex, age_cat,  read_write, enrolled_school, 
         school_level, highest_educ, study_site, occupation, transport_use) %>% # day_of_week, year,
...
first_survey_date <- contact_all %>%
              ...
              mutate(
                pand_period = case_when(
                  dt_diaryd1 <= as.Date("2021-6-1") ~ "lull1",
...
```{r}
contact_behav <- contact_all %>% select(study_site, age_cat, touch_contact, where_contact, contact_mask, duration_contact)%>%
              ...
              group_by(study_site, age_cat, var, value) %>%
              summarise(n=n()) %>% 
              left_join(contact_all %>% 
                          group_by(age_cat, study_site) %>%
                          summarise(tot_contacts=n()), by = c("age_cat"="age_cat", "study_site" = "study_site")) %>%
              mutate(prop = n/tot_contacts)

contact_behav %>% filter(var=="contact_mask") %>% 
  ggplot(x=age_cat, y =prop, fill = value) +
  geom_col(aes(x=age_cat, y =prop, fill = value), position = "fill") + facet_wrap(~study_site)+
...
contact_all_summary <- contact_all %>%
                        filter(Day1or2 ==1) %>%
                        group_by(rec_id,study_site,age_part, pand_period, pand_period_simp, pand_period_simp1)%>%
...
```{r}
contact_behav <- contact_all %>% select(study_site, pand_period, touch_contact, where_contact, contact_mask, duration_contact)%>%
                        mutate(pand_period = factor(pand_period, 
                                                    levels = c("lull1","inc_delta","dec_delta","lull2","inc_omi","dec_omi","lull3")))%>%
              mutate(where_contact = as.character(where_contact),
                     touch_contact = as.character(touch_contact),
              ...
              left_join(contact_all %>% 
                          ...
                          summarise(tot_contacts=n()), by = c("pand_period"="pand_period", "study_site" = "study_site")) %>%
              mutate(prop = n/tot_contacts)
...
pop_weight <- pop_weight %>% left_join(pop_dist, by = c("weight_cat"="age_cat","study_site"="study_site")) %>%
...
contact_all <- contact_all %>%
              ...
              left_join(
                pop_weight %>% select(study_site, weight_cat, part_weight,tot_pop),
...
contact_all <- contact_all %>%
                ...
                mutate(
                  ...
                  age_contact = as.numeric(age_contact),
...
```{r}
## urban
m_u <- contact_matrix(df_u, age.limits = c(0,18,50,99),symmetric=T, missing.contact.age="sample",estimated.contact.age = "sample",
                     weights= df_u$weights,n=1000)
...
m_r <- contact_matrix(df_r, age.limits = c(0,18,50,99),symmetric=T, missing.contact.age="sample",estimated.contact.age = "sample",
                     weights= df_r$weights,n=1000)
...
m_r1 <- contact_matrix(df_r, age.limits = c(0,18,50,99),symmetric=T, missing.contact.age="sample",estimated.contact.age = "sample",
                     n=1000)
