...
sum(!ind$rec_id %in% contact_hh$rec_id) # 84
...
ind <- ind %>%
    filter(!rec_id %in% unlist(missing_nonhh$rec_id)) 
## Dropped an additional 28 without any diaries (but had exit interviews, all from polana canico)
```
...
missing_hh %>% filter(rec_id %in% unlist(live_alone$rec_id))
...
contact_all <- plyr::rbind.fill(contact_nonhh %>% 
                                  select(rec_id,survey_date, study_site, Day1or2, 
                                ...
                                contact_hh %>% 
                                  select(rec_id, survey_date,study_site, Day1or2, hh_member_age, 
