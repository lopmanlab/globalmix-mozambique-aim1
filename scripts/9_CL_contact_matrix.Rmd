...
```{r}
contact_behav <- contact %>% select(study_site, contact_age, touch_contact, where_contact, contact_mask, duration_contact)%>%
              ...
              group_by(study_site, contact_age, var, value) %>%
              summarise(n=n()) %>% 
              left_join(contact %>% 
                          group_by(contact_age, study_site) %>%
                          summarise(tot_contacts=n()), by = c("contact_age"="contact_age", "study_site" = "study_site")) %>%
              mutate(prop = n/tot_contacts)

contact_behav %>% filter(var=="contact_mask") %>% 
  ggplot(x=contact_age, y =prop, fill = value) +
  geom_col(aes(x=contact_age, y =prop, fill = value), position = "fill") + facet_wrap(~study_site)+
...
contact_all <- contact %>% 
              left_join(first_survey_date, by="rec_id") %>%
              filter(study_day ==1) 
...
contact <- contact %>% 
  mutate(part_age = as.numeric(part_age),
         age_contact = as.numeric(contact_age),
...
cnt_u <- contact %>% filter(
                  study_site == "Polana Caniço"
)

cnt_r <- contact %>% filter(
                  study_site == "Manhiça"
)
