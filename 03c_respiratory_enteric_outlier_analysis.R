...
daily_resp_mean_model <- glm(daily_resp_mean_outlier ~ age + sex + site, 
                      data = contacts_resp_ent, family = binomial())
daily_resp_mean <- as.data.frame(summary(daily_resp_mean_model)$coefficients) %>%
  as.data.frame() %>%
  tibble::rownames_to_column()

daily_ent_mean_model <- glm(daily_ent_mean_outlier ~ age + sex + site, 
                             data = contacts_resp_ent, family = binomial())
...
daily_resp_q75_model <- glm(daily_resp_q75_outlier ~ age + sex + site, 
                             data = contacts_resp_ent, family = binomial())
daily_resp_q75 <- as.data.frame(summary(daily_resp_q75_model)$coefficients) %>%
  as.data.frame() %>%
  tibble::rownames_to_column()

daily_ent_q75_model <- glm(daily_ent_q75_outlier ~ age + sex + site, 
                            data = contacts_resp_ent, family = binomial())
...
png("outlier-analysis/daily_resp_mean.png", width=3000, height=1000, res=300)
...
png("outlier-analysis/daily_ent_mean.png", width=3000, height=1000, res=300)
...
png("outlier-analysis/daily_resp_q75.png", width=3000, height=1000, res=300)
...
png("outlier-analysis/daily_ent_q75.png", width=3000, height=1000, res=300)
